...
const PetSelector = ({ setSelectedPets, setShowResult }) => {
  ...
  const [selectedPets, setLocalSelectedPets] = useState([]); // 선택된 환수 리스트
  ...
  const handleTabChange = (tab) => {
    setSelectedTab(tab);
    setLocalSelectedPets([]); // 선택한 환수 초기화
  };
  ...
  return (
    <div className="pet-container">
      ...
      <div className="tab-buttons">
        <button className={selectedTab === "수호" ? "active" : ""} onClick={() => handleTabChange("수호")}>
          수호
        </button>
        <button className={selectedTab === "변신" ? "active" : ""} onClick={() => handleTabChange("변신")}>
          변신
        </button>
      </div>
